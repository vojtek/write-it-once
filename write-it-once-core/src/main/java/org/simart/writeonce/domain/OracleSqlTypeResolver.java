...
public class OracleSqlTypeResolver implements ColumnTypeResolver {
    ...
    @Override
    public String getFullType(Column column, Class<?> typeClass, Field field, Method method) {
    ...
    @Override
    public String getType(Column column, Class<?> typeClass, Field field, Method method) {
	if (String.class.isAssignableFrom(typeClass)) {
	    return "VARCHAR";
	}
	if (BigDecimal.class.isAssignableFrom(typeClass)) {
